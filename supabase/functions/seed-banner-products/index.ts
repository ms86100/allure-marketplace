...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Step 1: Delete existing banner section products for these sections
    const sectionIds = [
    ...
    for (const sid of sectionIds) {
      const { error } = await supabase.from("banner_section_products").delete().eq("section_id", sid);
      if (error) results.push(`delete ${sid}: ${error.message}`);
    }

    // Step 2: Insert products into sections
    const rows = [
    ...
    if (error) {
      results.push(`insert: ${error.message} (code: ${error.code})`);
    } else {
      results.push(`inserted ${data?.length} banner section products`);
    }

    // Step 3: Seed additional coupons
    const coupons = [
    ...
    for (const c of coupons) {
      const { error: ce } = await supabase.from("coupons").upsert(c, { onConflict: "code" });
      if (ce) results.push(`coupon ${c.code}: ${ce.message}`);
      else results.push(`coupon ${c.code}: ok`);
    }
